...
import { useKlineList, useMarketKlineWs } from "@/services/perps/market/hooks";
import type { MarketKlineRsp } from "@/services/perps/market/types";

const LightweightKlineChart = dynamic(
  () =>
    import("@/components/perps/lightweight-kline-chart").then(
...
export function PerpsChartPanel({
  ...
  return (
    <section
      ...
      aria-label={t("perps.chart")}
    >
      <div className="border-border/60 flex items-center border-b px-3 py-2 sm:px-4">
        <span className="text-foreground text-sm font-medium">{t("perps.chart")}</span>
      </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                ...
                aria-label={t("perps.chartMoreIntervals")}
              >
      ...
      <div className="relative min-h-0 flex-1 p-2 sm:p-3">
        {!chartReady && (
          <div className="bg-muted/40 flex h-full min-h-[220px] items-center justify-center rounded-lg">
            <p className="text-muted-foreground text-xs">{t("perps.chartLoading")}</p>
          </div>
        )}
        ...
        {chartReady && !isLoading && isError && (
          <div className="flex h-full min-h-[220px] items-center justify-center rounded-lg">
            <p className="text-muted-foreground text-xs">{t("perps.chartError")}</p>
          </div>
        )}

        {chartReady && !isLoading && !isError && bars.length === 0 && (
          <div className="flex h-full min-h-[220px] items-center justify-center rounded-lg">
            <p className="text-muted-foreground text-xs">{t("perps.chartEmpty")}</p>
          </div>
        )}
        ...
        {showChart && (
          <>
            ...
            {volumeStats ? (
              ...
              >
                <ChartOhlcStat
                  label={t("perps.chartVol").replace("{symbol}", baseSymbol)}
                  value={volumeStats.baseVolume}
                  valueClassName={volTone}
                />
                <ChartOhlcStat
                  label={t("perps.chartVol").replace("{symbol}", quoteSymbol)}
