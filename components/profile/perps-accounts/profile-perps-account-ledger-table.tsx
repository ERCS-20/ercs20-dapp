...
import { usePerpsAccountLedgerPagination } from "@/services/perps/accounts/hooks";
import type { PerpsAccountLedgerRsp } from "@/services/perps/accounts/types";
...
function LedgerRow({ row, symbol }: { row: PerpsAccountLedgerRsp; symbol: string }) {
  const { t } = useI18n();
...
export function ProfilePerpsAccountLedgerTable({
  tokenAddress,
  symbol,
}: {
  tokenAddress: string;
  ...
  const { data, isLoading, isFetching } = usePerpsAccountLedgerPagination(paginationReq, {
  ...
  return (
    <ProfilePaginatedTableSection
      ...
      footerColSpan={5}
      pageJumpId="perps-ledger-page-jump"
