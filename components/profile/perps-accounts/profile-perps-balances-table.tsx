...
import { usePerpsUserBalancesList } from "@/services/perps/accounts/hooks";
import type { PerpsUserBalancesRsp } from "@/services/perps/accounts/types";
...
function BalanceRow({ row }: { row: PerpsUserBalancesRsp }) {
  const { t } = useI18n();
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        <Link
          href={ProfileRoutes.perpsAccountDetail(row.tokenAddress)}
          className={cn(
...
export function ProfilePerpsBalancesTable() {
  const { t } = useI18n();
  const { isAuthenticated } = useAuth();
  const { data, isLoading } = usePerpsUserBalancesList({ enabled: isAuthenticated });

  const balances = data ?? [];

  return (
    <section className={profileTableSectionClass}>
      <h2 className="text-foreground text-base font-medium">{t("profile.perpsAccountList")}</h2>

      {!isAuthenticated ? (
        <p className="text-muted-foreground mt-6 text-sm">{t("profile.emptyPerpsBalances")}</p>
      ) : isLoading ? (
