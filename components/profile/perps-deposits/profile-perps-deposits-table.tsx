...
import {
  usePerpsDepositsPagination,
  usePerpsUserBalancesList,
} from "@/services/perps/accounts/hooks";
import type { PerpsDepositsRsp } from "@/services/perps/accounts/types";
...
function DepositRow({ row }: { row: PerpsDepositsRsp }) {
  const { t } = useI18n();
...
export function ProfilePerpsDepositsTable() {
  const { t } = useI18n();
  ...
  const { data, isLoading, isFetching } = usePerpsDepositsPagination(paginationReq, {
  ...
  return (
    <ProfilePaginatedTableSection
      title={t("profile.perpsDepositHistory")}
      showFilters={showFilters}
      isLoading={isLoading}
      totalCount={totalCount}
      hasRows={rows.length > 0}
      hasFilters={hasFilters}
      emptyMessage={t("profile.perpsDepositHistoryEmpty")}
      footerColSpan={6}
      pageJumpId="perps-deposit-page-jump"
    ...
    >
      <TableBody>
        {rows.map((row, index) => (
          <DepositRow
            key={`${row.tokenAddress}-${row.fromAddress}-${row.confirmedAt ?? index}`}
            row={row}
          />
        ))}
      </TableBody>
