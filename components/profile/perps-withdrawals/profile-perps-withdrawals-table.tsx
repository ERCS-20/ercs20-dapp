...
import { executeGlobalPerpsVaultWithdraw } from "@/lib/contracts/global-perps-vault";
...
import { getPerpsWithdrawalDetail } from "@/services/perps/accounts/api";
import {
  usePerpsUserBalancesList,
  usePerpsWithdrawalsPagination,
} from "@/services/perps/accounts/hooks";
import type { PerpsWithdrawalsRsp } from "@/services/perps/accounts/types";
...
function WithdrawalClaimButton({
  ...
  async function handleClaim() {
    ...
    try {
      const detail = await getPerpsWithdrawalDetail({ id: row.id });
      ...
      if (orderId == null || amount == null || !sysSignature) {
        toast.error(t("profile.claimFailed"));
        return;
      }

      const hash = await executeGlobalPerpsVaultWithdraw({
        publicClient,
        account: address,
        writeContractAsync,
        vaultAddress,
...
export function ProfilePerpsWithdrawalsTable() {
  const { t } = useI18n();
  ...
  const { data: balancesList } = usePerpsUserBalancesList({ enabled: isAuthenticated });
  ...
  const { data, isLoading, isFetching } = usePerpsWithdrawalsPagination(paginationReq, {
  ...
  function handleClaimSuccess() {
    void queryClient.invalidateQueries({ queryKey: ["perps", "accounts", "withdrawals"] });
    void queryClient.invalidateQueries({ queryKey: ["perps", "accounts"] });
  }

  return (
    <ProfilePaginatedTableSection
      title={t("profile.perpsWithdrawHistory")}
      showFilters={showFilters}
      isLoading={isLoading}
      totalCount={totalCount}
      hasRows={rows.length > 0}
      hasFilters={hasFilters}
      emptyMessage={t("profile.perpsWithdrawHistoryEmpty")}
      footerColSpan={7}
      pageJumpId="perps-withdrawal-page-jump"
