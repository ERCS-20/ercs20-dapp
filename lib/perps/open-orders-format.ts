...
import { pairLabelFromCode } from "@/lib/perps/pair-api";
import { apiBigIntToString, parseApiBigInt, type ApiBigInt } from "@/lib/utils/coerce-bigint";
import type { OrdersHistoryRsp, OrdersRsp, OrdersTradeHistoryRsp } from "@/services/perps/orders/types";
...
export function formatOpenOrderStatus(
  ...
  if (status === "New") return t("perps.statusOpen");
  if (status === "Cancelling") return t("perps.cancelling");
  return status;
}
...
export function formatOrderHistoryStatus(
  ...
  if (status === "Completed") return t("perps.statusFilled");
  if (status === "Canceled") return t("perps.statusCancelled");
  if (status === "PartialCanceled") return t("perps.statusPartial");
  return status;
}
...
export function formatTradeStatus(status: string, t: (key: string) => string): string {
  if (status === "Matched") return t("perps.tradeStatusMatched");
  if (status === "Settling") return t("perps.tradeStatusSettling");
  if (status === "Settled") return t("perps.tradeStatusSettled");
  if (status === "NotFound") return t("perps.tradeStatusNotFound");
  return status;
}
