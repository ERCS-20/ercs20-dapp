...
function toPair(t: Ercs20TokenMeta): PerpsPair {
  return {
    baseSymbol: t.symbol,
    baseName: t.name,
...
export function getMockMarketStats(pair: PerpsPair): PerpsMarketStats {
...
export function getMockOrderBook(pair: PerpsPair): PerpsOrderBook {
...
export function getMockMarketTrades(pair: PerpsPair): PerpsMarketTrade[] {
  const seed = hashSeed(pair.pairCode);
  const { lastPrice } = getMockMarketStats(pair);
  const trades: PerpsMarketTrade[] = [];
  for (let i = 0; i < 20; i++) {
...
export function getMockUserTradeHistory(pair: PerpsPair): PerpsUserTrade[] {
