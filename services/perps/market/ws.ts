...
import { getPerpsMarketWsUrl } from "@/lib/config/public-env";
import type {
  MarketWsChannel,
...
/**
 * Native WebSocket client for `perps-market-ws` (`/ws/market`).
 * Ref-counted subscribe; app-level JSON ping every 30s; exponential reconnect.
 */
class PerpsMarketWsClient {
  ...
  isConfigured(): boolean {
    return getPerpsMarketWsUrl().length > 0;
  }
...
export const perpsMarketWs = new PerpsMarketWsClient();
